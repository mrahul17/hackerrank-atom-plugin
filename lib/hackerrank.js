...
let regexp2 = /meta content="(.*)" *=name="csrf-token" /
...
export default {
  ...
  checkRequirements(){
    let auth_json = JSON.parse(fs.readFileSync(os.homedir()+'/config.json', 'utf8'))
    if(auth_json.username==undefined || auth_json.password==undefined){
      return false
    }
    else {
      auth.login = auth_json.username
      auth.password = auth_json.password
      return true;
    }
  },
  showError(e){
  ...
  login() {
    return new Promise((resolve, reject)=> {
      ...
      if(!this.checkRequirements()){
        ...
        'Please create a config.json file in your home folder with Hackerrank username and password'})
        reject()
        return;
      }
...
make_submission(details){
  ...
  return new Promise((resolve, reject)=> {
    request({url: problemUrl, method: "GET"}, (e,r,b)=>{
      ...
      else if (b.match(regexp2) != null) {
        csrf = b.match(regexp2)[1];
      }
