  ...
  return new Promise((resolve, reject)=> {
    request({url: problemUrl, method: "GET"}, (e,r,b)=>{
      ...
      request({url: submissionUrl, method: "POST", json: details.requestJson, headers: diffHeaders}, (e,r,b) => {
        ...
        a = {'submissionID':  submissionID,'submissionUrl': submissionUrl,'submission_type': details.submission_type }
        resolve(a)
      });
    });
  });
...
get_submission_status(details){
  ...
  compile(){
    this.login().then((status)=> {
        ...
        this.make_submission(details).then((status)=>{
          if(!status)
            return;
          this.get_submission_status(status)
